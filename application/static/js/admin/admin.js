...
window.onload = function() {
	...
	niceditor = new nicEditor({
		buttonList: ['bold', 'italic', 'underline', 'left', 'center', 'right', 'ol', 'ul', "removeformat", 'xhtml']
	});
	...
	addButtons.each(function(index) {
		...
		while (elCount < selectedElements.length - 1) {
			...
			elCount = elCount + 1;
			var orderField = $(thisEl).find(".form-row .field-order input")[0];
			if (!orderField) {
				orderField = $(thisEl).find(".form-row .field-order input")[0];
			}
			toGoInTable.push({
				"element": thisEl,
				"value": orderField
